"""MK Institutional Risk Analytics v0.08.
...
def infer_periodicity(index: pd.DatetimeIndex) -> tuple[int, str]:
    ...
    if med < 18*3600:
        ...
        bars_day=max(1,int(round(float(counts.median())))) if len(counts) else max(1,int(round(6.5*60/minutes)))
        return int(252*bars_day), f'Intraday ({minutes}m)'
    if med <= 3*86400:
        return 252,'Daily'
    if med <= 10*86400:
        return 52,'Weekly'
...
def rolling_window_options(index: pd.DatetimeIndex) -> list[RollingWindowSpec]:
    ...
    if freq.startswith('Intraday'):
        ...
        specs=[
            ...
            RollingWindowSpec('1W',bpd*5,ppy,freq),
            RollingWindowSpec('1M',bpd*21,ppy,freq),
            RollingWindowSpec('2M',bpd*42,ppy,freq),
        ]
    elif ppy==252:
