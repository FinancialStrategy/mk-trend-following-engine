"""
MK Yahoo Session-Aware Strict Adapter v0.08.3
By Murat Konuklar
...
- multiple Yahoo retrieval paths reconcile when their observed values agree within an explicitly bounded machine-precision envelope
- accepted values always remain actually observed Yahoo values; no averaging or synthetic consensus price is created
...
class YahooAudit:
    ...
    precision_normalized_matches: int
    max_precision_spread_bps: float
    precision_examples: list[dict]
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "precision_normalized_matches": self.precision_normalized_matches,
            "max_precision_spread_bps": self.max_precision_spread_bps,
            "precision_examples": self.precision_examples,
        }
...
# Precision envelope for same-source Yahoo reconciliation.
# Direct Yahoo Chart JSON and yfinance can expose the same adjusted price through
# slightly different float serialization paths. Exact binary equality is therefore
# not economically meaningful. This is deliberately a MACHINE-PRECISION envelope,
# not a market-price tolerance.
#
# 16 * eps(float32) ~= 1.907e-6 relative = 0.0191 basis points.
# At TRY 100 that is about TRY 0.00019, far below one kurus.
FLOAT32_ULP_MULTIPLIER = 16.0
PRICE_ABS_TOL_FLOOR = 2.0e-6


def _price_precision_tolerance(reference: float) -> float:
    ref = abs(float(reference))
    ulp_envelope = FLOAT32_ULP_MULTIPLIER * np.finfo(np.float32).eps * max(ref, 1.0)
    return float(max(PRICE_ABS_TOL_FLOOR, ulp_envelope))


def _agreement_diagnostics(values: list[float], column: str) -> dict:
    if len(values) <= 1:
        v = float(values[0]) if values else np.nan
        return {
            "agree": True, "reference": v, "min": v, "max": v,
            "absolute_spread": 0.0, "relative_spread": 0.0,
            "spread_bps": 0.0, "tolerance": 0.0,
        }

    arr = np.asarray(values, dtype=float)
    reference = float(arr[0])  # highest-priority actual Yahoo observation
    abs_spread = float(np.nanmax(arr) - np.nanmin(arr))
    denom = max(abs(reference), np.finfo(float).tiny)
    rel_spread = abs_spread / denom
    spread_bps = rel_spread * 10000.0

    if column == "Volume":
        tolerance = 0.5
        agree = bool(np.all(np.isclose(arr, reference, rtol=0.0, atol=tolerance)))
    else:
        tolerance = _price_precision_tolerance(reference)
        agree = bool(np.all(np.abs(arr-reference) <= tolerance))

    return {
        "agree": agree, "reference": reference,
        "min": float(np.nanmin(arr)), "max": float(np.nanmax(arr)),
        "absolute_spread": abs_spread, "relative_spread": rel_spread,
        "spread_bps": spread_bps, "tolerance": float(tolerance),
    }


def _values_agree(values: list[float], column: str) -> bool:
    return bool(_agreement_diagnostics(values, column)["agree"])
...
def _consensus_frame(
    ...
    *,
    ...
    now_utc: pd.Timestamp | None = None,
) -> tuple[pd.DataFrame, list[str], dict[str, list[str]], str, list[dict]]:
    ...
    for idx in target_index:
        for c in REQUIRED_COLUMNS:
            ...
            if observed:
                diag = _agreement_diagnostics(observed, c)
                if not diag["agree"]:
                    conflicts.append(
                        f"{idx} {c}: Yahoo routes materially disagree {observed[:4]} "
                        f"(spread={diag['absolute_spread']:.12g}, "
                        f"{diag['spread_bps']:.6f} bps; allowed={diag['tolerance']:.12g})"
                    )
                else:
                    # Preserve an ACTUAL Yahoo observation from the highest-priority route.
                    # Never average, median, interpolate, or manufacture a consensus value.
                    out.at[idx, c] = observed[0]
                    if len(observed) > 1 and diag["absolute_spread"] > 0:
                        precision_matches.append({
                            "timestamp": str(idx), "column": c,
                            "reference_value": diag["reference"],
                            "min_value": diag["min"], "max_value": diag["max"],
                            "absolute_spread": diag["absolute_spread"],
                            "spread_bps": diag["spread_bps"],
                            "allowed_tolerance": diag["tolerance"],
                            "routes_compared": len(observed),
                        })
    ...
    return out, unresolved, detail, cutoff, precision_matches
...
class YahooFinanceAdapter:
    ...
    def fetch(
        ...
        try:
            consensus, unresolved, detail, cutoff, precision_matches = _consensus_frame(
        ...
        if unresolved and _is_bist(ticker) and interval == "1d":
            ...
            consensus2, unresolved2, detail2, cutoff2, precision_matches2 = _consensus_frame(
                routes, ticker=ticker, interval=interval, start=str(start), end=str(end)
            )
            ...
            precision_matches = precision_matches2
        ...
        audit = YahooAudit(
            ...
            precision_normalized_matches=len(precision_matches),
            max_precision_spread_bps=max(
                [float(x["spread_bps"]) for x in precision_matches] or [0.0]
            ),
            precision_examples=precision_matches[:12],
        )
